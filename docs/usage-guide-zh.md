...
出站 HTTPS Webhook 会先在本机解析并校验目标，然后使用已校验的公网 IP 作为代理
`CONNECT` 请求中的 authority（连接目标）。只接受主机名的企业代理可能拒绝该 IP
字面量。GTA-Claw 会将其明确报告为策略固定目标被代理拒绝，且不会改用主机名重试，
因为那会把 DNS 解析和目标策略委托给代理。
...
### Q: 为什么本地节点发现显示 mDNS 不可用？
GTA-Claw 不会把“成功创建 mDNS 守护进程”当作“发现功能可用”的证据。开始浏览或
广播前，必须通过本机网络栈实际观察到一次 IPv4 组播查询和响应，并在消费能力令牌
时再次观察；发现记录仍可解析 IPv4 和 IPv6 端点。探测超时或之后发生接口移除时，
会明确报告不可用，而不是返回空节点列表。请检查组播防火墙策略和路由所选择的活动
接口；GTA-Claw 不会把这种情况解释成“零个节点”。
