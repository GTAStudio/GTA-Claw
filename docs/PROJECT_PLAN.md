...
模型缓存年龄后续：显式 `core.provider.catalogue_max_age_ms` 限制为1秒到24小时，省略保留
旧行为；单调时钟准入在到期/未知年龄时拒绝新的文本、流式、嵌入和运行时请求，别名不能绕过。

同一记录的后续回归又证明：过期请求在最终准入前会更新本地历史访问时间。已将年龄检查
前移至模型解析，并保留调用前复查；原始失败保留，daemon全目标259项通过/5项忽略，
严格lint及一项最终历史回归通过。进入时已过期的请求不续期历史；准备期间才到期的请求
仍不调用provider，但不宣称全部准备操作可回滚。
取消或无效刷新不续期，成功显式刷新同锁更新目录与时间。CLI `models --freshness`、TUI和桌面
状态入口显示查询时年龄/上限，不加入稳定摘要或导出，不声称账号就绪。三方言实际到期与恢复、
五包641项通过/7项忽略、桌面98项通过，双workspace严格lint和根check通过，见
[缓存年龄记录](ledger/native-model-cache-20260916.json)。已准入请求不追溯取消，系统挂起跨平台、
实号撤权与原凭据异常仍开放；无自动刷新或新任务完成声明。

离线用量估算增量：`accounting estimate` 固定完整导出和显式费率卡两个文件SHA，复用既有
快照全量校验，按精确provider/model和整数定点费率计算已知小计；缺失/部分计数或费率不补零，
缓存/推理子集不重复收费，不读取旧浮点价格。真实多页导出到估算的13种非空/1种空文件场景、
CLI全目标117项通过/2项既有忽略、严格lint和根check通过，见
[离线估算记录](ledger/native-accounting-estimate-20260916.json)。真实价格适用性、发票核对、
货币预留/硬限额、GUI估算及原凭据异常仍开放，不改变服务端未结算用量或运行状态。
