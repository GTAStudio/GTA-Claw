...
| `src/engine/toolExecutor.ts` | Tool registry and disposal. JavaScript evaluation and its domain-allowlisted HTTP/log bridge have already been removed from Node (not ported) as emergency hardening; the class only tracks skill-registration metadata and every execution attempt is rejected. | `claw-tools` + `claw-skills` | **Partial by deliberate break (JS evaluation and bridge already removed from Node).** Safe, non-executable bridge behavior — if ever reintroduced — still requires typed Rust tools and signed Rust/WASI skill ports; it must never become a JavaScript engine. |
...
| `src/updater/sdkUpdater.ts` | Nonblocking installed/latest version reporting only. Package-manager (`npm update`) and curl self-mutation have already been removed from Node as emergency hardening; `checkForUpdates` now only logs a report-only warning. | No assigned signed-release updater | **Owner required.** Package-manager and curl self-mutation are deliberately forbidden and have already been removed from Node; only verified signed releases may replace this reporting for Rust. |
...
| `isolated-vm` and `node:vm` | None — already removed from Node by emergency hardening. (Previously: execute remotely supplied JavaScript skills.) | Already removed; never reintroduce, and never add an embedded JavaScript engine to Rust. |
