...
async fn cumulative_outbound_bytes_bound_multiple_requests_behind_stalled_writer() {
    const HELD_ID: &str = "budget-held";
    const REJECTED_IDS: [&str; 2] = ["budget-rejected-one", "budget-rejected-two"];
    let held_params = json!({"value": "x".repeat(1024)});
    let rejected_params = json!({"value": "y".repeat(512)});
    ...
    let outbound_bytes = held_bytes.max(rejected_bytes[0]).max(rejected_bytes[1]);
    assert!(held_bytes <= outbound_bytes);
    for bytes in rejected_bytes {
        assert!(bytes <= outbound_bytes);
        assert!(
            held_bytes + bytes > outbound_bytes,
    ...
    let scenario = tokio::spawn(async move {
        ...
        assert!(response.ok(), "held request must succeed: {response:?}");
