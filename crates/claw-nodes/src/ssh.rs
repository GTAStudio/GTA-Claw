...
/// # Errors
///
/// Returns [`SshError`] when the challenge is invalid or the signing identity
/// does not match the discovered node.
...
/// # Errors
///
/// Returns [`SshError`] when the challenge, identity binding, signature age, or
/// node signature is invalid.
...
impl SshTarget {
    ...
    /// # Errors
    ///
    /// Returns [`SshError::InvalidTarget`] when the target is empty, ambiguous,
    /// malformed, or could inject an SSH option.
...
impl SshConfig {
    ...
    /// # Errors
    ///
    /// Returns [`SshError`] when the configuration exceeds its fixed bound or a
    /// supported directive is malformed.
    pub fn parse(contents: &str) -> Result<Self, SshError> {
        ...
        for raw_line in contents.lines() {
            ...
            match directive.to_ascii_lowercase().as_str() {
                ...
                "hostname" => set_once(
                    &mut blocks.last_mut().ok_or(SshError::InvalidConfig)?.host_name,
                    value,
                ),
                "user" => set_once(
                    &mut blocks.last_mut().ok_or(SshError::InvalidConfig)?.user,
                    value,
                ),
                "port" => {
                    let port = parse_port(value)?;
                    let slot = &mut blocks.last_mut().ok_or(SshError::InvalidConfig)?.port;
                    if slot.is_none() {
                        *slot = Some(port);
                    }
                }
    ...
    /// # Errors
    ///
    /// Returns [`SshError::InvalidTarget`] when the alias is empty or contains
    /// whitespace.
...
/// # Errors
///
/// Returns [`SshError`] when the host is unknown, the key changed, or the
/// selected known-hosts file cannot be checked.
...
impl SshConnection {
    ...
    /// # Errors
    ///
    /// Returns [`SshError`] when user or key validation, network connection,
    /// host-key verification, or public-key authentication fails.
    pub async fn connect(
        ...
        let private_key = russh::keys::load_secret_key(
            ...
            key_passphrase.map(secrecy::ExposeSecret::expose_secret),
        )
    ...
    /// # Errors
    ///
    /// Returns [`SshError::Io`] when the listener cannot bind.
    ...
    /// # Errors
    ///
    /// Returns [`SshError`] when the forwarding target is invalid, the listener
    /// or SSH channel fails, a forwarding task fails, or the stop channel closes.
    ...
    /// # Errors
    ///
    /// Returns [`SshError`] when the forwarding target, SSH channel, or
    /// bidirectional copy fails.
    ...
    /// # Errors
    ///
    /// Returns [`SshError`] when the bind address is invalid, the SSH request
    /// fails, or the allocated port is not representable.
    ...
    /// # Errors
    ///
    /// Returns [`SshError`] when the listener identity is invalid or the SSH
    ...
    /// # Errors
    ///
    /// Returns [`SshError::ConnectionClosed`] when the bounded forwarded-channel
    /// queue closes.
    ...
    /// # Errors
    ///
    /// Returns [`SshError`] when the SSH disconnect request fails.
...
impl ForwardedConnection {
    ...
    /// # Errors
    ///
    /// Returns [`SshError`] when the destination is invalid, connection fails,
    /// or bidirectional copying fails.
...
mod tests {
    ...
    impl server::Handler for LoopbackSshHandler {
        ...
        async fn channel_open_direct_tcpip(
            ...
            let Ok(port_to_connect) = u16::try_from(port_to_connect) else {
                reply
            ...
            let Ok(mut target) = TcpStream::connect((host_to_connect, port_to_connect)).await
            else {
