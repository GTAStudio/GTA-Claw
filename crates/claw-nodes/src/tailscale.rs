...
impl LoopbackLocalApiTransport {
    ...
    /// # Errors
    ///
    /// Returns [`TailscaleError::UnsafeEndpoint`] when the URL is credentialed,
    /// non-HTTP, non-loopback, or has no usable port.
...
impl TailnetStatus {
    ...
    /// # Errors
    ///
    /// Returns [`TailscaleError::IncompleteIdentity`] when any required node,
    /// address, user, or DNS identity field is absent.
...
impl TailscaleAcl {
    ...
    #[must_use]
    pub const fn new(rules: Vec<TailscaleAclRule>) -> Self {
        Self { rules }
    }
    ...
    /// # Errors
    ///
    /// Returns [`TailscaleError::PermissionDenied`] when no complete allow rule
    /// matches both the actor and operation.
    pub fn authorize(
        &self,
...
impl<T: LocalApiTransport> TailscaleClient<T> {
    ...
    /// # Errors
    ///
    /// Returns [`TailscaleError`] when the bounded `LocalAPI` request or response
    /// validation fails.
    ...
    /// # Errors
    ///
    /// Returns [`TailscaleError`] when ACL authorization or status discovery
    /// fails.
    ...
    /// # Errors
    ///
    /// Returns [`TailscaleError`] when ACL authorization, address parsing,
    /// `LocalAPI` resolution, or identity completeness checks fail.
    pub async fn whois(
        &self,
        actor: &WhoIsIdentity,
        address: &str,
    ...
    /// # Errors
    ///
    /// Returns [`TailscaleError`] when ACL authorization, exposure validation,
    /// identity discovery, compare-and-swap configuration, or bounded retries
    /// fail.
    // Holding this lock across LocalAPI awaits serializes configuration and bookkeeping.
    ...
    ///
    /// Returns [`TailscaleError`] when ACL authorization, exposure validation,
    /// restart adoption, compare-and-swap configuration, or bounded retries
    /// fail.
    // Holding this lock across LocalAPI awaits serializes configuration and bookkeeping.
    #[allow(clippy::significant_drop_tightening)]
    ...
    async fn acl_denies_partial_identity_and_prevents_api_call() {
        ...
        let client = TailscaleClient::new(
            ...
            TailscaleAcl::new(vec![TailscaleAclRule {
                ...
                permissions: std::iter::once(TailscalePermission::Discover).collect(),
            }]),
        );
