...
mod tests {
    ...
    async fn open_transaction_quota_preserves_unrelated_cleanup_headroom() {
        ...
        .await
        .expect_err("seven holders, twenty-five waiters, and eight probes exhaust capacity");
        for owner in final_headroom {
            owner
                .shutdown()
                .expect("complete final headroom cleanup reservation");
        }
    ...
    async fn open_waits_for_earlier_verifier_retirement_before_seven_set() {
        ...
        let earlier_headroom = claw_sqlite_file_control::BlockingCleanupOwner::acquire_set(
            "earlier-verifier-headroom-proof",
            63,
            std::time::Instant::now() + Duration::from_secs(1),
        )
        .await
        .expect("only the gated earlier verifier remains charged");
        claw_sqlite_file_control::BlockingCleanupOwner::acquire_set(
            "earlier-verifier-overcommit-proof",
            1,
            std::time::Instant::now() + Duration::from_millis(50),
        )
        .await
        .expect_err("the gated earlier verifier owns the final cleanup slot");
        for owner in earlier_headroom {
            owner
                .shutdown()
                .expect("release earlier verifier headroom proof");
        }
        ...
        let peak_headroom = claw_sqlite_file_control::BlockingCleanupOwner::acquire_set(
            "newest-verifier-headroom-proof",
            56,
            std::time::Instant::now() + Duration::from_secs(1),
        )
        .await
        .expect("the seven-set and newest verifier hold exactly eight cleanup slots");
        claw_sqlite_file_control::BlockingCleanupOwner::acquire_set(
            "newest-verifier-overcommit-proof",
            1,
            std::time::Instant::now() + Duration::from_millis(50),
        )
        .await
        .expect_err("the newest verifier eight-owner peak exhausts remaining capacity");
        for owner in peak_headroom {
            owner
                .shutdown()
                .expect("release newest verifier headroom proof");
        }
        peak_release.fetch_add(1, std::sync::atomic::Ordering::AcqRel);
        opening
            .await
    ...
    async fn open_actor_survives_caller_runtime_drop_before_and_after_construction() {
        ...
        let all = claw_sqlite_file_control::BlockingCleanupOwner::acquire_set(
            "runtime-drop-owner-capacity-proof",
            64,
            std::time::Instant::now() + Duration::from_secs(2),
        )
        .await
        .expect("caller runtime drops retire every cleanup owner");
        for owner in all {
            owner
                .shutdown()
                .expect("release runtime-drop capacity proof");
        }
    }
