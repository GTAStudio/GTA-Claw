...
impl ManualClock {
    ...
    #[expect(
        ...
        reason = "tests call this for the side effect of moving time forward; the returned \
                  timestamp is a convenience for the assertions that need it, so requiring \
                  every caller to bind it would be noise"
    )]
