//! Integration coverage for optional file-backed durable memory and transcripts.
...
fn state_files(root: &Path, collection: &str) -> Vec<PathBuf> {
    let mut files = fs::read_dir(root.join(collection))
...
fn only_state_file(root: &Path, collection: &str) -> PathBuf {
    let files = state_files(root, collection);
...
fn corrupt_backups(root: &Path, collection: &str) -> Vec<PathBuf> {
    let mut backups = fs::read_dir(root.join(collection))
        .expect("state collection exists")
        .map(|entry| entry.expect("read state entry").path())
        .filter(|path| {
            path.file_name()
...
fn durable_memory_is_idempotent_addressable_restartable_and_scope_isolated() {
    ...
    assert_eq!(
        state_files(root.path(), "memory").len(),
        1,
...
fn durable_bounds_fail_without_evicting_existing_data() {
    ...
    memory
        .add(&scope, MemoryTarget::UserProfile, "profile", 3)
...
#[test]
fn corrupt_and_overbounded_state_is_quarantined_byte_for_byte() {
    ...
    let mut document: Value = serde_json::from_slice(
        &fs::read(only_state_file(root.path(), "memory")).expect("read recreated state"),
    )
    .expect("valid recreated state");
    document["scope"] = Value::String("a".repeat(65));
    let overbounded_scope =
        serde_json::to_vec_pretty(&document).expect("serialize overbounded document");
    fs::write(&memory_path, &overbounded_scope).expect("inject overbounded scope");
    assert!(
        memory
            .list(&scope, MemoryTarget::Memory, 0, 20)
            .expect("bounded wire recovers")
            .entries
            .is_empty()
    );
    assert!(
        corrupt_backups(root.path(), "memory")
            .iter()
            .any(|backup| fs::read(backup).expect("read backup") == overbounded_scope)
    );
    ...
    assert_eq!(
        fs::read(&corrupt_backups(root.path(), "transcripts")[0]).expect("read transcript backup"),
        corrupt_transcript
    );
}
...
fn stale_write_temporaries_are_removed_under_the_scope_lock() {
    ...
    fs::write(&stale, vec![0_u8; 1_024]).expect("create simulated crash leftover");
...
#[test]
fn reduced_configs_preserve_data_without_exposing_over_capacity_context() {
...
fn state_documents_are_bound_to_their_scope() {
    ...
    store
        ...
        .expect("write beta");
    let beta_path = state_files(root.path(), "memory")
        .into_iter()
        .find(|path| *path != alpha_path)
...
fn cross_process_writer_helper() {
    ...
    for _ in 0..2_000 {
        if Path::new(&start).exists() {
            DurableMemoryStore::new(root, 5_000, 100)
                .expect("child store")
                .add(
...
#[test]
#[cfg(unix)]
fn ambient_alias_is_canonicalized_and_descendant_links_are_rejected() {
    use std::os::unix::fs::symlink;

    let sandbox = TempDir::new("path-links");
    ...
    let runtime =
        DurableStateRuntime::open(DurableStateConfig::new(&configured_root, 500, 500, 10, 500))
            .expect("open through ambient alias");
    let scope = scope("ambient-alias");
    runtime
        .memory()
        .add(&scope, MemoryTarget::Memory, "alias-backed memory", 1)
        .expect("write through ambient alias");
    ...
    let collection_link = canonical_root.join("transcripts");
    symlink(&external, &collection_link).expect("link transcript collection");
    assert!(matches!(
        runtime
            .transcript()
            .append(&scope, TranscriptRole::User, "must remain confined", 2),
        Err(TranscriptError::Persistence(_))
    ));
    assert_eq!(
...
fn linked_state_is_rejected_without_quarantining_external_bytes() {
    ...
    let external_bytes = br#"{"must":"remain external"}"#;
...
#[test]
#[cfg(unix)]
fn newly_created_state_is_private_to_the_current_user() {
    use std::os::unix::fs::PermissionsExt;

    let root = TempDir::new("permissions");
    let store = DurableMemoryStore::new(root.path(), 100, 100).expect("valid limits");
    store
        .add(
            &scope("private-state"),
            MemoryTarget::Memory,
        ...
        .expect("write state");
    let mode = fs::metadata(only_state_file(root.path(), "memory"))
