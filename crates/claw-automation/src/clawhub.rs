...
impl<S: ArtifactStore> ClawHubLifecycle<S> {
    ...
    #[must_use]
    pub fn new(store: S, trust: TrustPolicy) -> Self {
        Self {
            store,
            trust,
    ...
    fn manifest(version: &str, risky: bool) -> ReleaseManifest {
        ReleaseManifest {
            ...
            capabilities: if risky {
                [PackageCapability::Network].into_iter().collect()
            } else {
                [PackageCapability::FilesystemRead].into_iter().collect()
            },
        }
    }
    ...
    fn filesystem_store_separates_case_distinct_package_ids() {
        ...
        let release = |package: PackageName, artifact: &[u8]| HubRelease {
            manifest: ReleaseManifest {
                ...
                capabilities: [PackageCapability::FilesystemRead].into_iter().collect(),
            },
            artifact: artifact.to_vec(),
