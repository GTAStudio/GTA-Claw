...
impl WebhookSecret {
    ...
    /// # Errors
    ///
    /// Returns [`WebhookError::WeakSecret`] when the secret is shorter than 32
    /// bytes.
...
impl InboundRoute {
    ...
    /// # Errors
    ///
    /// Returns [`WebhookError`] when the route, action allowlist, body bound, or
...
impl ReplayGuard {
    ...
    /// # Errors
    ///
    /// Returns [`WebhookError::InvalidReplayPolicy`] when the window is zero,
    /// exceeds 24 hours, cannot be represented, or the capacity is zero.
    ...
    const fn validate_timestamp(&self, timestamp: i64, now: i64) -> Result<(), WebhookError> {
        if now.abs_diff(timestamp) > self.window_seconds.unsigned_abs() {
...
impl InboundWebhookHandler {
    ...
    /// # Errors
    ///
    /// Returns [`WebhookError`] when a route is invalid, paths collide, or no
    /// route is configured.
    ...
    /// # Errors
    ///
    /// Returns [`WebhookError`] when authentication, replay admission,
    /// authorization, decoding, or application dispatch fails.
...
/// # Errors
///
/// Returns [`WebhookError`] when the nonce, canonical input, or signing state is
/// invalid.
pub fn sign_webhook(
...
impl OutboundEndpoint {
    ...
    /// # Errors
    ///
    /// Returns [`WebhookError::UnsafeEndpoint`] when the endpoint violates the
    /// selected transport policy.
    pub fn validate(&self, allow_loopback_http: bool) -> Result<(), WebhookError> {
        let valid_scheme = self.url.scheme() == "https"
            || (allow_loopback_http
                ...
                && self.url.host().as_ref().is_some_and(loopback_host));
        if self.id.is_empty()
            || self.id.len() > 128
            || !valid_scheme
...
const fn loopback_host(host: &url::Host<&str>) -> bool {
    match host {
...
impl RustlsOutboundTransport<SystemWebhookResolver> {
    ...
    /// # Errors
    ///
    /// Returns [`WebhookError`] when the timeout or native TLS root
    ...
    async fn send_inner(
            ...
            && addresses.iter().all(|address| is_public_ip(address.ip()))
        {
            ...
            if let Some(proxy) = self.proxy.intercept(&uri) {
                // Public addresses are globally scoped, so the proxy can dial the exact address
                // that passed local policy without re-resolving the destination hostname.
                return self
...
fn validate_resolved_addresses(
    ...
    let permitted = if url.scheme() == "http" {
        url.host().as_ref().is_some_and(loopback_host)
...
const fn is_connectable_ip(address: IpAddr) -> bool {
    match address {
        IpAddr::V4(address) => {
...
impl RetryPolicy {
    ...
    /// # Errors
    ///
    /// Returns [`WebhookError::InvalidRetryPolicy`] when the attempt count or
    /// delay bounds are invalid.
    pub fn validate(&self) -> Result<(), WebhookError> {
        if self.max_attempts == 0
            || self.max_attempts > 10
            || self.base_delay.is_zero()
            || self.base_delay > self.max_delay
            || self.max_delay > Duration::from_hours(1)
        {
            return Err(WebhookError::InvalidRetryPolicy);
        }
        Ok(())
...
pub trait WebhookClock: Send + Sync {
    ...
    /// # Errors
    ///
    /// Returns [`WebhookError::Clock`] when the wall clock cannot produce a
    ...
    S: RetrySleeper,
{
    ...
    /// # Errors
    ///
    /// Returns [`WebhookError::InvalidRetryPolicy`] when the retry policy is
    /// invalid.
    pub fn new(
        transport: T,
        dead_letters: D,
    ...
    C: WebhookClock,
{
    ...
    /// # Errors
    ///
    /// Returns [`WebhookError`] when endpoint or event validation, signing,
    /// transport, retry timing, or dead-letter persistence fails.
        ...
        for attempt in 1..=self.retry.max_attempts {
            ...
            match self.transport.send(request).await {
                ...
                Err(error) if error.retryable_delivery_failure() => {
                    "transport failure".clone_into(&mut last_failure);
                }
                Err(error) => return Err(error),
            }
...
impl WebhookError {
    const fn retryable_delivery_failure(&self) -> bool {
        matches!(
            self,
    ...
    fn handler(max_body_bytes: usize) -> InboundWebhookHandler {
        InboundWebhookHandler::new(
            vec![InboundRoute {
                ...
                path: "/plugins/webhooks/taskflow".to_owned(),
                secret: secret(),
                allowed_actions: std::iter::once("task.completed".to_owned()).collect(),
                session_binding: SessionBinding::Prefix("taskflow:".to_owned()),
                max_body_bytes,
            }],
            ReplayGuard::new(Duration::from_mins(5), 8).expect("replay"),
        )
        .expect("handler")
    }
