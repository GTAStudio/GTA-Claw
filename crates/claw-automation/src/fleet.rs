...
const MAX_LEASE_TTL: Duration = Duration::from_mins(5);
...
impl CellSpec {
    ...
    /// # Errors
    ///
    /// Returns [`FleetError`] when the cell, a member, the image, or the health
    /// policy is invalid, or when no coordinator is configured.
    pub fn validate(&self) -> Result<(), FleetError> {
        if !valid_identifier(&self.id)
            ...
            || self.health_timeout > Duration::from_mins(10)
        {
...
impl CliContainerRuntime {
    ...
    /// # Errors
    ///
    /// Returns [`FleetError::InvalidRuntime`] when the executable path is empty.
...
impl<R: ContainerRuntime> FleetController<R> {
    ...
    /// # Errors
    ///
    /// Returns [`FleetError`] when the lease request is invalid or conflicts
    ...
    /// # Errors
    ///
    /// Returns [`FleetError`] when the lease is stale, mismatched, expired, or
    /// requests an invalid lifetime.
    pub fn renew_lease(
        &mut self,
    ...
    /// # Errors
    ///
    /// Returns [`FleetError::LeaseMismatch`] when the lease is no longer the
    /// exact active lease.
    ...
    /// # Errors
    ///
    /// Returns [`FleetError`] when validation, lease admission, runtime setup,
    /// member startup, health checks, or transactional rollback fails.
    pub async fn create(
        &mut self,
        spec: CellSpec,
        lease: &OperationLease,
    ...
    /// # Errors
    ///
    /// Returns [`FleetError`] when the cell is absent, runtime health cannot be
    /// read, or controller state is inconsistent.
    ...
    /// # Errors
    ///
    /// Returns [`FleetError`] when the cell or log bound is invalid, the runtime
    /// fails, or returned output exceeds the fixed limit.
    pub async fn logs(
        &self,
        cell_id: &str,
        tail_lines: u16,
    ...
    /// # Errors
    ///
    /// Returns [`FleetError`] when the lease or destination is invalid, the cell
    /// is absent, or a member backup fails.
    pub async fn backup(
        &self,
        cell_id: &str,
        destination: PathBuf,
    ...
    /// # Errors
    ///
    /// Returns [`FleetError`] when cell status cannot be collected.
    ...
    /// # Errors
    ///
    /// Returns [`FleetError`] when lease or image validation fails, the runtime
    /// cannot update a member, or a replacement is unhealthy.
    pub async fn rolling_update(
        ...
        let record = self
            ...
            .get_mut(cell_id)
            .ok_or(FleetError::CellNotFound)?;
        image.clone_into(&mut record.spec.image);
    ...
    /// # Errors
    ///
    /// Returns [`FleetError`] when the lease or cell is invalid, runtime
    /// teardown fails, or controller state is inconsistent.
    pub async fn remove(
        &mut self,
        cell_id: &str,
        lease: &OperationLease,
...
mod tests {
    ...
    async fn full_create_status_logs_backup_doctor_remove_lifecycle() {
        ...
        let lease = controller
            .acquire_lease("studio", "operator", 100, Duration::from_mins(5))
            .expect("lease");
    ...
    fn operation_leases_conflict_expire_and_require_exact_tokens() {
        ...
        let first = controller
            ...
            .expect("first");
        assert!(matches!(
            controller.acquire_lease("studio", "other", 101, Duration::from_mins(5)),
            Err(FleetError::LeaseConflict)
        ));
        let second = controller
            .acquire_lease("studio", "other", 401, Duration::from_mins(5))
