...
use claw_application::composition::ServiceHandle;
...
use crate::ingress::{HttpIngress, http_ingress_id};
use crate::services::{self, HttpServices};
use crate::settings::{self, DaemonSettings};
...
pub async fn await_stop(
    ...
    // The parking for an exhausted control channel happens *inside* this
    // future, not in the `select!` handler. A handler runs only after its
    // branch has completed, at which point `select!` has already stopped
    // polling the others — so parking there would swallow the signal branch
    // and leave the process unable to observe SIGTERM at all. That is the
    // production case: a systemd service inherits `/dev/null` on standard
    // input unless the unit says otherwise, so the channel reaches end of
    // file immediately.
    let control = async {
        match await_control(control).await {
...
pub async fn serve(
    ...
    // Configuration is resolved before any subsystem is built, so an invalid
    // environment stops the process rather than starting it half-configured.
    ...
    report_manual_migrations(&settings);

    let ingress = HttpIngress::new(
        settings.listen_address(),
        HttpApi::new(api_config(), HttpServices::new().services()),
    );
    let mut daemon = Daemon::builder()
        .with_subsystem(Arc::new(ingress))
        .build()?;

    let handles = daemon.start().await?;

    writeln!(output, "{}", application.ready())?;
    writeln!(output, "{}", application.health())?;
    for address in handles
        .iter()
        .filter(|handle| handle.subsystem() == &http_ingress_id())
        .flat_map(ServiceHandle::bound)
    {
        writeln!(
            output,
            "listening address={address} domain={}",
            settings.public_domain()
        )?;
    }
    report_closed_surfaces(&mut output)?;
    output.flush()?;
...
///
/// No bearer credential is installed. Resolving one requires credential
/// resolution, a durable audit sink and provider adapters, which land together
/// as one security-reviewed unit; installing a token before then would widen a
/// legacy administrator secret into a credential for every protected route.
fn api_config() -> ApiConfig {
    ApiConfig::new(BearerAuthenticator::default())
}

/// Reports legacy values this runtime intentionally cannot apply.
fn report_manual_migrations(settings: &DaemonSettings) {
...
/// Names the surfaces that answer without doing work, so an operator learns it
/// from startup rather than from a 401 or a 503.
fn report_closed_surfaces(mut output: impl Write) -> io::Result<()> {
    writeln!(
        output,
        "unconfigured dependencies={} ready=false",
        services::UNCONFIGURED_DEPENDENCIES.join(",")
    )?;
    writeln!(
        output,
        "protected routes closed: no bearer credential is configured"
    )
}
