...
shell_surfaces=(
  ...
  "$SCRIPT_DIR/package-container.sh" \
...
python3 "$SCRIPT_DIR/tests/reject-javascript-commands.py" "${shell_surfaces[@]}"
printf '%s\n' \
  '#!/bin/sh' \
...
printf '%s\n' \
  'FROM scratch' \
  'RUN "/usr/bin/node" build.js' \
  >"$tmp_dir/javascript-Dockerfile"
printf '%s\n' \
  '[Service]' \
  'ExecStartPre=/usr/bin/npm install' \
  >"$tmp_dir/javascript.service"
if python3 \
  ...
  "$tmp_dir/javascript.service" \
  >/dev/null 2>&1; then
  echo "shell JavaScript-command policy accepted quoted or env-wrapped commands" >&2
  exit 1
fi
