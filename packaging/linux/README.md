...
RPM reports some `%post` failures as warnings; the RPM hook therefore leaves
`/run/gta-claw-initialization-failed` on any failed handoff, and the runtime
unit refuses startup until a later successful root initialization clears it.
