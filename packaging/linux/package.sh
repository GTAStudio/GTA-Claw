...
GTA_CLAW_INSTALLED_VERSIONS
  printf '%%s\n' "\$PPID" >/run/gta-claw-daemon.replacement
...
if [ -d /run/systemd/system ]; then
  if [ "\$1" -eq 0 ]; then
    ...
    replacement_owner="\$(cat /run/gta-claw-daemon.replacement)"
    if [ "\$replacement_owner" = "\$PPID" ]; then
      if [ -e /run/gta-claw-daemon.was-active ]; then
        test -e /run/gta-claw-daemon.ready-for-replacement
        systemctl is-active --quiet gta-claw-daemon.service
      fi
      touch /run/gta-claw-daemon.old-removal-succeeded
    fi
  fi
fi
exit 0
...
%posttrans
set -e
if [ -e /run/gta-claw-daemon.old-removal-succeeded ]; then
  rm -f /run/gta-claw-daemon.was-active
fi
rm -f \
  /run/gta-claw-daemon.old-removal-succeeded \
