...
require_tool docker
[[ "$#" -eq 4 ]] ||
  die "usage: native-self-test-container.sh ARCH NATIVE_ARCHIVE BUILD_MANIFEST EXPECTED_BUILD_KEY_SHA256"
arch="$1"
archive="$2"
manifest="$3"
expected_key_sha="$4"
: "${OUTPUT_ROOT:?OUTPUT_ROOT must select a new native mutation root}"
target_root="$(canonical_target_root)"
for input in "$archive" "$manifest"; do
  validate_absolute_path "$input" "native self-test input"
  [[ "$input" == "$target_root/"* ]] || die "native self-test input is outside target"
done
...
docker run --rm \
  ...
  -- \
  bash -c '
    exec ./packaging/linux/native-self-test.sh \
      "$1" \
      "$2" \
      "$BUILD_MANIFEST" \
      "$3"
...
[[ "$(stat -Lc '%d:%i' "$REPO_ROOT")" == "$repo_mount_id" ]] ||
  die "repository path identity changed during native mutation transaction"
[[ "$(stat -Lc '%d:%i' "$target_root")" == "$target_mount_id" ]] ||
  die "target path identity changed during native mutation transaction"
