...
fn run_activated_repository_policy_tests(tree: &TempTree) {
    ...
    for directory in ["cargo-home", "target", "temp"] {
    ...
    copy_directory(
        ...
        &isolation.join(REPOSITORY_POLICY_MEMBER),
    )
    .expect("copy activated repository policy into execution workspace");
    fs::write(
        isolation.join("Cargo.toml"),
        r#"[workspace]
members = ["crates/claw-repo-policy"]
resolver = "3"
...
"#,
    ...
    .expect("write isolated repository policy workspace");
    fs::write(
        isolation.join("Cargo.lock"),
        "# This file is automatically @generated by Cargo.\nversion = 4\n\n[[package]]\nname = \"claw-repo-policy\"\nversion = \"0.1.0\"\n",
    )
    .expect("write isolated repository policy lock");
